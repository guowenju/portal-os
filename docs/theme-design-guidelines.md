...
本文档定义 PortalOS 前端主题接入规范。PortalOS 以 `animal-island-ui` 的动物岛视觉语言为基准：暖羊皮纸表面、圆润胶囊轮廓、柔和棕色描边、青绿色主操作、珊瑚色危险操作和具有纵深感的阴影。新增应用必须优先组合组件库并使用全局主题 token，避免形成第二套视觉系统。

## 组件库优先级

- 组件库已有的交互控件不得重复实现，组件属性以已安装包的 `AI_USAGE.md` 为唯一依据。
- 不覆盖主按钮的 3D 底部阴影，不抹平组件圆角，不使用通用企业后台式直角灰白面板。
- 自定义编辑器、Markdown 阅读区和复杂布局只能补足组件库缺失能力，并必须复用下方 PortalOS token。
- 组件库没有公开 CSS token；PortalOS token 是外围布局的适配层，不用于重绘组件库内部结构。
...
- 没有让窗口内容区露出浅色硬编码背景。
- 优先使用组件库组件，且所有属性都存在于 `AI_USAGE.md`。
- 未覆盖主按钮 3D 阴影、胶囊圆角或组件库核心配色。
- 在浅色、深色、桌面、最小窗口、移动端和减少动效模式下检查 header、内容区、弹窗和滚动区域。
- 完成键盘操作、焦点可见性和基础屏幕阅读器标签检查。
